...
public class QRViewCodeAuth : MonoBehaviour {

	List<string> msgs = new List<string>();

	public Texture2D qrCodeImage;
	string url = "http://localhost:8000/cs_req_auth_qr2?service_id=123";
	...
	void HandleResponse (HTTP.Response response)
	{
		...
		if ( o != null )
		{
			...
			string code=(string)t["sc_res_auth_qr2"];
			qrCodeImage = QRCodeProcessor.Encode((string)code, _width, _height);
			
			QRAuthPoll poll=GetComponent<QRAuthPoll>();
			if ( poll )
			{
				//poll.id=
				poll.qrCode=code;
